...
Ugly, not flexible, but easy to use and always available.
...
`reshape2` is another package which must be installed, also by Hadley Wickham, and often used in conjunction with ggplot2.

+ data must have right "shape" before plotting
+ (also very useful tool to have for other purposes)
...
```{r, fig.width=8, fig.height=4}
ggplot(diamonds, aes(price, carat)) + geom_point() + scale_y_log10()
```
...
+ doesn't have normal R docs
+ _does_ have excellent online documentation
