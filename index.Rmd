...
## Exercises: Auto-Mpg Data

1. Dataset: Auto-Mpg Data from UCI Machine Learning Repository
...
3. Variables (names saved in auto-mpg-names.txt): 
  ...
  + string: car name (unique for each instance)
  + descriptions: mpg (miles per gallon), cylinders (# of cylinders), displacement (engine displacement in cu. inches), weight (vehichle weight in lbs.), accelerate (time to accelerate from O to 60 mph in sec.), model year (modulo 100), origin (1: American, 2: European, 3: Japanese).
4. Goal: The data concerns city-cycle fuel consumption in miles per gallon (mpg), to be predicted in terms of 3 multivalued discrete and 5 continuous attributes.
5. More information:   https://archive.ics.uci.edu/ml/datasets/Auto+MPG
