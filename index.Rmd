...
# The exercises will generally follow the typical process of a statistical analysis. 


# Suggestion for the exercises
+ First work on the regular problems (with no "optional" marks). 
...
EX5. Subset exercises 
  ...
  + 5.4 read the help file for which(); create a vector of indices for the instances (rows) where origin is 2 or 3; create a new data that contains only the instances where origin is 2 or 3. Hint: logical operator: & for and, | for or.
...
EX6. Discrete variables 
  ...
  + 6.2 use a loop to print the name and the frequency table for each discrete variable. Note that there are very few instances with odd number of cylinders. We will revisit this. Also... -99 again? 
  ...
  + 6.6 convert variable origin into factor type, and add the labels for the values (1: American, 2: European, 3:Japanese). 
  + 6.7 check the variable type of the converted variable and the variable from the original copy of the data
  + 6.8 table() on the new variable and see how the output changes
...
EX7. Missing values
  ...
  + 7.3 using is.na() function, take a subset of the dataset that has non-missing mpg 
  and check the dimension of the subset. Hint: !is.na() refers to NOT NA. 
  + 7.4 (optional) create a vector "contain_no_NA", with each element being a logical value indicating whether it has all non-missing values, for each instance. Hint: use all() to return whether the statement is true for all elements of a vector. (note: actually complete.cases() can do the above job directly.)
    + a) use for loop to get contain_no_NA (loop over the rows of data)
    + b) use apply() to get contain_no_NA
    + d) use contain_no_NA to get the new data that contains only the subset that has no missing values. 
  + 7.5 read the help file for function na.omit(), and use this function to create a new data that contains only the instances that has no missing variables

---

# From now on use the data set that contains no missing values
EX8. (optional) Exercises for *apply() and split()
  + 8.1 use one of the *apply() functions to get the frequency table on all discrete variables. Note that this can be directly done by summary(data[,c(2,7,8)]) if these variables are all of factor type instead of numerical type
...
EX9. Randomization testing for whether mean mpg is higher for Japanese cars than European cars
...
EX11. Boxplot of mpg by different levels of originto visually check if mpg is different across different categories
...
13. Scatterplot matrix to check the relationship between any pair of variable, to check for linearity assumption and homogeneity assumption. If violated, need to do data transformation. 
...
EX14. Data transformation
  ...
  + Add new variables to the data: 
    + log transformed versions of mpg, horsepower, displacement, and weight; 
...
EX15. Statistical analysis  
  ...
  + 15.2 linear regression. Build a linear regression model to predict mpg. Include all the variables; if a transformed version is available for a variable, use only the transformed version)
  + 15.3 Apply summary() on the regression object and read the output. Is origin still helpful in predicting mpg after including other predictors? Which column is to answer the relationship between mpg and other variables? 
  + 15.4 Diagnosis (important in statistical analysis). Execute plot(model) in R and check the four plots. You can find the specific help file for "plot()" for a lm object by executing "?plot.lm". Based on the diagnostic plots, are these reasonable models? Which one is better? Check the following aspects.  
